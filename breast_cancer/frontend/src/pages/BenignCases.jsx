...
export default function BenignCases() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const benign = predictionsRes.data.filter(p => p.prediction === "Benign");
        setCases(benign);
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-green-600">Benign Cases</h1>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            {cases.length === 0 ? (
              <p className="text-gray-600">No benign cases found.</p>
            ) : (
